...
//    private List<String> images;//图片经过压缩处理
    ...
    public MyCallBack(RecyclerView.Adapter adapter,  List<String> originImages,String addPicPath) {
...
//            return true;
//        }
        if(originImages.get(toPosition).contains(addPicPath)){
            return true;
        }
...
//                Collections.swap(images, i, i - 1);
    ...
    public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if (!originImages.contains(addPicPath)) {
            originImages.add(addPicPath);
        }
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        System.out.println("outLocation"+outLocation[0]+"..."+outLocation[1]);
        if(outLocation[1]+viewHolder.itemView.getHeight() >= bottomY){
//        if (dY >= (recyclerView.getHeight()
//                - viewHolder.itemView.getBottom()//item底部距离recyclerView顶部高度
//                - viewHolder.itemView.getContext().getResources().getDimensionPixelSize(R.dimen.post_moment_img_delete))) {//拖到删除处
            ...
            if (up) {//在删除处放手，则删除item
                ...
                String removePath = originImages.remove(viewHolder.getAdapterPosition());
//                images.remove(viewHolder.getAdapterPosition());

                final int removePosition = viewHolder.getAdapterPosition();
...
//                adapter.notifyDataSetChanged();
                dragListener.onDelete(removePath);
                initData();
                return;
            }
        } else {//没有到删除处
    ...
    public interface DragListener {
        ...
        void onDelete(String removePath);
    }
