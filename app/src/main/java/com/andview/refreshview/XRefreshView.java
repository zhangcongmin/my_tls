...
//        if(mHolder.mOffsetY!=0)return;
        ...
        int childCount = getChildCount();
        int top = (int) (getPaddingTop() + mHolder.mOffsetY);
        int adHeight = 0;
        for (int i = 0; i < childCount; i++) {
            View child = getChildAt(i);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (deltaY > 0 && mHolder.mOffsetY <= mHeadMoveDistence || deltaY < 0) {
                    Log.i("XRefreshView","currentY=" + currentY + ";mLastY=" + mLastY+";deltaY="+deltaY);
                ...
                if (mHolder.hasHeaderPullDown()) {
                    ...
                    if (!mStopingRefresh) {
                        if (mEnablePullLoad && !isEmptyViewShowing() && needAddFooterView() && !mHasLoadComplete) {
                            ...
                            float offset = 0 - mHolder.mOffsetY;
    ...
    public boolean invokeLoadMore() {
        if (mEnablePullLoad && !isEmptyViewShowing() && !mPullRefreshing && !mStopingRefresh && !mHasLoadComplete) {
            float offset = 0 - mHolder.mOffsetY - mFootHeight;
            if (offset != 0) {
                startScroll(offset, Utils.computeScrollVerticalDuration(offset, getHeight()));
            }
            startLoadMore();
     ...
     * @param during
     */
    private void updateHeaderHeight(float currentY, float deltaY, int... during) {
    ...
    private void updateFooterHeight(float deltaY) {
        if (mEnablePullLoad) {
            if (needAddFooterView()) {
...
//        mFooterCallBack.onStateRefreshing();
     ...
     */
    public void startScroll(float offsetY, int duration) {
        mScroller.startScroll(0, (int) mHolder.mOffsetY, 0, (int) offsetY, duration);
        post(mRunnable);
    }
    ...
    private ScrollRunner mRunnable = new ScrollRunner() {
        ...
        public void run() {
            if (mScroller.computeScrollOffset()) {
                float lastScrollY = mHolder.mOffsetY;
                int currentY = mScroller.getCurrY();
                float offsetY = currentY - lastScrollY;
    ...
    public interface XRefreshViewListener {
         ...
         */
        void onHeaderMove(double headerMovePercent, float offsetY);
    }
    ...
    public static class SimpleXRefreshListener implements XRefreshViewListener {
        ...
        @Override
        public void onHeaderMove(double offset, float offsetY) {
        }
